...
module APB_Master (
    ...
    output logic        PSEL2,
    output logic        PSEL3,
    input  logic [31:0] PRDATA0,   // peripheral 갯수만큼 생성
    input  logic [31:0] PRDATA1,
    input  logic [31:0] PRDATA2,
    input  logic [31:0] PRDATA3,
    input  logic        PREADY0,   // peripheral 갯수만큼 생성
    input  logic        PREADY1,
    input  logic        PREADY2,
    input  logic        PREADY3,
    // Internal Interface Signals
    // transfer : CPU에서 APB 쪽으로 trigger 하는 신호 (trigger signal)
    ...
    always_comb begin
        ...
        case (state)
            IDLE: begin
                ...
                if (transfer) begin
                    next            = SETUP;
                    temp_addr_next  = addr;  // latching : 임시로 저장
    ...
    APB_Decoder U_APB_Decoder (  // memory mapping
        .en (decoder_en),
        .sel(temp_addr_reg),
        .y  (pselx)
    );
